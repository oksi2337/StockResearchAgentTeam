...
포맷 규칙 (매우 중요):
- **bold(`**`)를 절대 사용하지 마세요.** 소제목도 평문으로 작성.
- 링크는 반드시 [제목](url) 형식으로 제목 텍스트에 삽입. `([링크])` 형식 금지.
...
## 🛠️ 새로 나온 도구 3
1. [도구명](링크) — [한 줄 설명] / 써볼 이유: [한 줄 평가]
2. [도구명](링크) — [한 줄 설명] / 써볼 이유: [한 줄 평가]
3. [도구명](링크) — [한 줄 설명] / 써볼 이유: [한 줄 평가]
